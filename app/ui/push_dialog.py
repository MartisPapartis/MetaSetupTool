"""Push progress dialog — shown while pushing updates to the API."""

from __future__ import annotations

from app.ui.base_progress_dialog import BaseProgressDialog
from app.workers.push_worker import PushWorker
from app.workers.worker_config import WorkerConfig


class PushProgressDialog(BaseProgressDialog):
    def __init__(self, config: WorkerConfig, parent=None):
        super().__init__(parent)
        self._validate_only = config.validate_only
        title = "Previewing Push" if config.validate_only else "Pushing Updates to API"
        self.setWindowTitle(title)
        self.setMinimumSize(720, 500)
        self.setModal(True)
        initial_status = "Previewing changes…" if config.validate_only else "Pushing updates..."
        self._setup_progress_ui(initial_status=initial_status)

        self.worker = PushWorker(config, parent=self)
    ...
    def _on_finished(self, success: bool, message: str) -> None:
        if success:
            ok_text = "Preview complete!" if self._validate_only else "Push complete!"
            ...
            err_text = (
                ...
                else "Push finished with errors."
            )
